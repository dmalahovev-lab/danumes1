...
// Инициализируем приложение Google Firebase
...
// Вход в систему (упрощенный, по уникальному никнейму, без бэкенда)
authBtn.addEventListener('click', () => {
    const username = document.getElementById('authUsername').value.trim().toLowerCase();
    if (!username) return alert('Введите ваш логин!');
    if (username.length < 3) return alert('Логин должен быть длиннее 2 символов');
...
function startApp() {
    ...
    // Запускаем прослушивание базы данных на появление новых сообщений
    listenToMessages();
    // Слушаем статусы "печатает..."
    listenToTyping();
}

// Кнопка принудительного старта чата по точному логину друга
startChatBtn.addEventListener('click', () => {
    const targetUser = userSearchInput.value.trim().toLowerCase();
    if (!targetUser) return alert('Введите логин друга!');
...
// Комната для двоих (сортировка ников)
...
// Отрисовка сообщений на экране чата
...
// Переключение реакций (добавить/убрать свой ник)
function sendReaction(msgId, emoji) {
    ...
    rRef.once('value', snapshot => {
        ...
        const index = currentUsers.indexOf(myUsername);
        if (index !== -1) {
            currentUsers.splice(index, 1);
        } else {
            currentUsers.push(myUsername);
        }
        rRef.set(currentUsers);
    });
}

// Настройка режимов ответа и изменения
...
// Живой статус "печатает..." через Realtime Database
...
function sendTypingStatus(isTyping) { 
    if (activeChatUser) { 
        database.ref('typing/' + activeChatUser + '/' + myUsername).set(isTyping); 
    } 
}

function listenToTyping() {
    database.ref('typing/' + myUsername).on('value', snapshot => {
        const statuses = snapshot.val();
        if (statuses && activeChatUser && statuses[activeChatUser]) {
